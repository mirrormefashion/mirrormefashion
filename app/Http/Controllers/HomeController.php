...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('frontend.shop', compact('featured_categories', 'todays_deal_products'));
    }
...
public function seller_newsfeeds(){
    $feeds = Post::orderBy('created_at', 'DESC')->get();
    return view('frontend.user.seller.news_feed', compact(['feeds']));
}
public function seller_blogs(){
    $blogs = Blog::where('status', 1)->where('user_id',auth()->user()->id)->orderBy('created_at', 'desc')->paginate(12);
    return view('frontend.user.seller.blog.index',compact('blogs'));
}
public function seller_blogs_create(){
    $blog_categories = BlogCategory::all();
    return view('frontend.user.seller.blog.create',compact('blog_categories'));
}
