...
document.getElementById("female-skinny-head").style.backgroundPosition = `0px 0px`;
document.getElementById("female-skinny-neck").style.backgroundPosition = `0px 0px`;
document.getElementById("female-skinny-breasts").style.backgroundPosition = `0px 0px`;
document.getElementById("female-skinny-shoulders").style.backgroundPosition = `0px 0px`;
document.getElementById("female-skinny-stomach").style.backgroundPosition = `0px -231px`;
document.getElementById("female-skinny-legs").style.backgroundPosition = `0px 0px`;



//Reconsile changes to each feature 
let femaleHead = document.querySelector("#female-skinny-head");
let femaleNeck = document.querySelector("#female-skinny-neck");
let femaleBreasts = document.querySelector("#female-skinny-breasts");
let femaleShoulders = document.querySelector("#female-skinny-shoulders");
let femaleStomach = document.querySelector("#female-skinny-stomach");
let femaleLegs = document.querySelector("#female-skinny-legs");

function defaultModel() {
    if (femaleHead != null) {
        femaleHead.style.backgroundPosition = `0px 0px`;
        femaleHead.style.bottom = `-113px`;
    ...
    if (femaleBreasts != null) {
        ...
        femaleBreasts.style.bottom = `45px`;
    ...
    if (femaleStomach != null) {
        femaleStomach.style.backgroundPosition = `0px -231px`;
...
function attrChecker() {
    ...
    if (slideValNeckHt != null) {
        if (slideValNeckHt == 0) {
            ...
            classification = "Average";
        } else if (slideValNeckHt == 5) {
            classification = "Short";
        } else if (slideValNeckHt == 6) {
            classification = "Short";
        } else if (slideValNeckHt == 7) {
...
function recHead() {
    ...
    if (slideValStomach == 0 && slideValShoulderHt >= 3) {
        x = -4329 + xpos;
        ypos = (-1185 * slideValArm) + (-237 * slideValShoulderHt);
        ...
        if (slideValShoulderHt == 0 || slideValShoulderHt == 2 || slideValShoulderHt == 4) {
            if (slideValShoulderHt == 2) {
                ...
                femaleHead.style.bottom = `${headBase + headPos}px`;
            }
        } else {
            n = -26;
    ...
    if (slideValShoulderHt == 0) {
        ...
        femaleNeck.style.backgroundPosition = `${k_xpos}px -125px`;

    } else if (slideValShoulderHt == 3) {
        femaleNeck.style.backgroundPosition = `${k_xpos}px -125px`;

    } else if (slideValShoulderHt == 4) {
        femaleNeck.style.backgroundPosition = `${k_xpos}px -250px`;
...
function recBreasts() {
    ...
    let slideValStomach = document.getElementById("slider-stomach-shape").value;
    let slideValBreasts = document.getElementById("slider-breasts-shape").value;
    y = (-232 * slideValBreasts);
    xpos = -481 * slideValShoulderWt;
    base = 45;
    decrement = 3;
    newPos = base - decrement;
    femaleBreasts.style.bottom = `${base}px`;

    if (slideValStomach == 0) {
        x = -4329 + xpos;
        if (slideValShoulderHt <= 1) {
            femaleBreasts.style.backgroundPosition = `${x}px ${y}px`;
        } else if (slideValShoulderHt == 2) {
            femaleBreasts.style.backgroundPosition = `${x}px ${-2088 + y}px`;
        } else {
            femaleBreasts.style.backgroundPosition = `${x}px ${-4176 + y}px`;
        }
    } else {
        if (slideValShoulderHt == 0) {
            femaleBreasts.style.backgroundPosition = `${xpos}px ${y}px`;
        } else if (slideValShoulderHt >= 1 && slideValShoulderHt <= 2) {
            femaleBreasts.style.backgroundPosition = `${xpos}px ${-2088 + y}px`;

        } else {
            femaleBreasts.style.backgroundPosition = `${xpos}px ${-4176 + y}px`;
        }
    }
    ...
    if (slideValArm == 0) {
        ...
        femaleBreasts.style.bottom = `${newPos}px`;
    }
}

function recShoulders(isCall = true) {
    let slideValShoulderWt = document.getElementById("slider-shoulder-width").value;
    let slideValShoulderHt = document.getElementById("slider-shoulder-height").value;
    let slideValArm = document.getElementById("slider-arm-size").value;
    let slideValStomach = document.getElementById("slider-stomach-shape").value;
    o_ypos = -231 * slideValStomach;
    ...
    if (slideValStomach == 0) { //rectangle stomach
        x = -4329 + xpos;
        femaleShoulders.style.backgroundPosition = `${x}px ${ypos}px`;
        document.getElementById("slider-shoulder-height").style.max = "2";
        if (slideValShoulderHt >= 3) {
            ...
            w = (-1185 * slideValArm) + (-237 * 2);
            femaleShoulders.style.backgroundPosition = `${x}px ${w}px`;
        } else {
            w = (-1185 * slideValArm) + (-237 * slideValShoulderHt);
            document.getElementById("slider-shoulder-height").value = slideValShoulderHt;
            femaleShoulders.style.backgroundPosition = `${x}px ${w}px`;
        }
    } else if (slideValStomach >= 1 && slideValStomach <= 3) {
        z_ypos = (-422 * slideValShoulderWt);
        femaleShoulders.style.backgroundPosition = `${xpos}px ${ypos}px`;
    } else {
        defaultModel();
    }
}
...
function recLegs() {
    ...
    if (slideValStomach == 0) { //rectangle
        z = -3798;
        z_ypos = z + (-422 * slideValShoulderWt);
...
function classify() {
    ...
    if (stomach == "Muffintop" || stomach == "Spoon" || stomach == "Rectangle") {
        if (bust >= 5) {
            document.getElementById("classification").setAttribute("value", stomach);
            document.getElementById("variant").setAttribute("value", "Voluptous");
        } else {
            document.getElementById("classification").setAttribute("value", stomach);
            document.getElementById("variant").setAttribute("value", "Classic");
        }
    } else {
        if (stomach == "Average" || stomach == "Curvy") {
            if (bust >= 5) {
