...
Route::get('/aiz-uploader/destroy/{id}', 'AizUploadController@destroy')->name('media-files.destroy');
...
Route::get('/password/reset/{email}/{code}', 'HomeController@reset_password')->name('password.reset');
...
Route::post('/advertiser/registration', 'AdvertiserController@create_advertiser')->name('advertiser.registration');
...
Route::group(['prefix' => 'seller', 'middleware' => ['seller', 'verified', 'user']], function() {
    ...
    Route::get('/newsfeeds', 'HomeController@seller_newsfeeds')->name('seller.newsfeeds');
    Route::get('/blogs', 'HomeController@seller_blogs')->name('seller.blogs'); 
    Route::get('/blogs/upload', 'HomeController@seller_blogs_create')->name('seller.blogs.create'); 
    Route::post('/blogs/store', 'BlogController@store')->name('seller.blogs.store'); 
...
Route::get('/blog/category/{slug}', 'BlogController@blog_category')->name('blog.category');

Route::get('/#MirrorMeFashion-Virtual_Stylist', 'UsersController@index')->name('signup');

Route::get('/',                          'UsersController@index')->name('home');

Route::get('/login',                     'UsersController@login')->name('login');
Route::post('/users/create',             'UsersController@store')->name('user.create');
Route::get('/update/maintenance-mode',             'UsersController@UpdateMaintenanceMode')->name('update.maintenance_mode');
Route::post('/users/login',              'UsersController@doLogin')->name('user.dologin');

//Route::get('/profile',                   'UsersController@profile')->name('profile');
Route::get('/users/update',              'UsersController@update')->name('user.update');
Route::post('/profile/upload-avatar',    'UsersController@uploadAvatar')->name('user.upload.avatar');
Route::post('/media/upload',             'UsersController@uploadMedia')->name('user.upload.media');
Route::get('/media/delete/{media_upload}','UsersController@removeMedia')->name('user.media.delete');
Route::get('/profile/delete/{user}',     'UsersController@destroy')->name('user.destroy');


Route::get('/about', 'PublicController@about');
Route::get('email-validation/{email}', 'PublicController@is_unique_mail')->name('email.is_unique');

Route::get('/alpha', 'PublicController@alpha')->name('alpha');
Route::get('/contact', 'PublicController@about')->name('about');
Route::get('/fashion-dictionary','PublicController@fashion');
Route::get('/privacy','PublicController@privacy');
Route::get('/terms','PublicController@terms')->name('term');
Route::get('/sizing-and-conversions-chart','PublicController@metric_conversion')->name('metric_conversion');
Route::get('/feedback/{id}','feedbackController@create')->name('feedback.create');
Route::post('/feedback','feedbackController@store')->name('feedback.store');


Auth::routes();

// Allow reloading w/o shell access
...
Route::post('user/post-create','PostController@store')->name('create-post');
Route::get('user/post-remove/{id}','PostController@destroy')->name('remove-post');
Route::post('user/update-post/{id}','PostController@update')->name('update-post');
Route::post('user/post-report','PostController@report')->name('report-post');
Route::get('user/post/update/{id}','PostController@edit')->name('getpost-by-post');
Route::post('/posts/like', 'LikeController@store')->name('post.like');
Route::post('/posts/comment-like', 'LikeController@comment_like')->name('comment.like');
Route::get('user/posts/comment/{id}/destroy', 'CommentController@destroy')->name('comment.destroy');
Route::get('user/messanger/{user_name}','MessageController@index')->name('get-messanger');
Route::post('user/send-messange','MessageController@store')->name('send-message');
Route::post('user/send-comment','SocialController@commentStore')->name('comment.add');
Route::post('user/reply-comment','SocialController@commentReplyStore')->name('comment-reply.add');
Route::get('user/profile','SocialController@getUserProfile')->name('get_user_profile');
Route::post('user/profile/search-friends-ajax','SocialController@search_friends_ajax')->name('search-friends-ajax');
Route::post('user/profile/search-users','SocialController@search_users')->name('search-users');
Route::get('user/profile/settings','SocialController@settings')->name('get-user-settings');
Route::get('user/profile/friend-page','SocialController@friend_page')->name('get-friend-page');
Route::get('user/profile/messanger-page','SocialController@messanger_page')->name('get-messanger-page');
...
Route::post('user/privecy-setting-update','SocialController@privecy_setting_update')->name('user.privecy-setting-update');
Route::post('user/notification-setting-update','SocialController@notify_update')->name('user.is_notify');
Route::post('user/change-password','SocialController@change_password')->name('user.change-password');
Route::post('user/profile-picture-change','SocialController@profilePicChange')->name('user.profile-picture-change');
Route::post('user/links-update','SocialController@update_website_links')->name('user.website-links-update');
Route::get('profile/{user_name}','SocialController@index')->name('get-user-profile-page');
Route::post('profile/send-friend-request','FriendController@sendFriendRequest')->name('send-friend-request');
Route::post('profile/remove-friend','FriendController@removeFriend')->name('remove-friend');
Route::get('block-friend/{id}','FriendController@blockFriend')->name('block-friend');
Route::post('add-top-10-friend','FriendController@addTop10Friend')->name('add-top-10-friend');
});
